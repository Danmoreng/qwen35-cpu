...
int main(int argc, char ** argv) try {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (arg == "--cpu-batch") sequences = std::stoi(value());
  ...
  auto evaluate = [&](const llama_token * data, int count, int position, int sequence, bool logits) {
  ...
  for (int sequence = 0; sequence < sequences; ++sequence) {
    auto row_prompt = prompt;
    row_prompt.back() = (row_prompt.back() + sequence) % vocab;
    for (int position = 0; position < int(prompt.size());) {
      const int count = std::min(2048, int(prompt.size()) - position);
      evaluate(row_prompt.data() + position, count, position, sequence,
  ...
  if (!prefill_only) for (int i = 0; i + 1 < generated; ++i) {
    batch.n_tokens = sequences;
    for (int sequence = 0; sequence < sequences; ++sequence) {
      batch.token[sequence] = forced[i]; batch.pos[sequence] = int(prompt.size()) + i;
      batch.n_seq_id[sequence] = 1; batch.seq_id[sequence][0] = sequence;
      batch.logits[sequence] = true;
    }
    if (llama_decode(ctx.get(), batch) != 0) throw std::runtime_error("Batched llama_decode failed");
    llama_synchronize(ctx.get());
    capture(i+1);
  }
  ...
  if (!prefill_only) {
    std::ofstream final_dump;
    if (!final_logits_path.empty()) final_dump.open(final_logits_path, std::ios::binary);
    for (int sequence = 0; sequence < sequences; ++sequence) {
      const float * logits = llama_get_logits_ith(ctx.get(), sequence);
      ...
      if (!final_logits_path.empty()) {
        final_dump.write(reinterpret_cast<const char *>(logits), vocab * sizeof(float));
        if (!final_dump) throw std::runtime_error("Could not write final validation logits");
      }
    }
  }
  ...
  out << std::setprecision(12) << "{\"prefill_only\":" << (prefill_only ? "true" : "false")
      ...
      << ",\"prefill_tokens_per_second\":" << prompt.size() * sequences * 1000.0 / prefill_ms
      << ",\"decode_time_ms\":" << decode_ms
      << ",\"tokens_per_second\":" << (prefill_only ? 0.0 : (generated - 1) * sequences * 1000.0 / decode_ms)
