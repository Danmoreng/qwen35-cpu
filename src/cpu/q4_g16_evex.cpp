...
G16_INLINE void wide_accumulate(const WideHalf& w,const Q4G16ActivationTile* a,
    std::size_t b,std::size_t first,int half,__m512& acc) noexcept {
  const auto& v=a[b];const auto* q=v.qs[first+T]+half*16;
  ...
  dot=_mm512_sub_epi32(dot,_mm512_set1_epi32(8*v.sums[half][first+T]));
  const auto scale=_mm512_mul_ps(w.scales,_mm512_set1_ps(v.scales[first+T]));
  acc=_mm512_fmadd_ps(_mm512_cvtepi32_ps(dot),scale,acc);
}
template<std::size_t... T>
G16_INLINE void wide_tile(const Q4G16Tile* w,const Q4G16ActivationTile* a,
  ...
  for(std::size_t b=0;b<blocks;++b) {
    ...
    (wide_accumulate<T>(low,a,b,first,0,acc[T]),...);
    const auto high=wide_half(w[b],w[blocks+b],1);
    (wide_accumulate<T>(high,a,b,first,1,acc[T]),...);
  }
  (_mm512_storeu_ps(out+T*stride,acc[T]),...);
}
}
void q4_g16_matmul_wide(const Q4G16Tile* w,const Q4G16ActivationTile* a,
  ...
  for(std::size_t group=0;group<count;group+=16) for(std::size_t r=0;r<complete_rows;r+=16) {
    const auto n=std::min(std::size_t(16),count-group);std::size_t t=0;
    for(;t+8<=n;t+=8)
      wide_tile(w+(r/8)*blocks,a+(group/16)*blocks,out+(group+t)*stride+r,blocks,stride,t,std::make_index_sequence<8>{});
    for(;t+4<=n;t+=4)
      wide_tile(w+(r/8)*blocks,a+(group/16)*blocks,out+(group+t)*stride+r,blocks,stride,t,std::make_index_sequence<4>{});
    for(;t+2<=n;t+=2)
      wide_tile(w+(r/8)*blocks,a+(group/16)*blocks,out+(group+t)*stride+r,blocks,stride,t,std::make_index_sequence<2>{});
    for(;t<n;++t)
      wide_tile(w+(r/8)*blocks,a+(group/16)*blocks,out+(group+t)*stride+r,blocks,stride,t,std::make_index_sequence<1>{});
  }
  if(rows!=complete_rows)
    q4_g16_matmul_evex(w+(complete_rows/8)*blocks,a,out+complete_rows,rows-complete_rows,count,blocks,stride);
}
}
