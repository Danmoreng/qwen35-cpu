...
const char* q4_g16_batch_kernel_name(Q8_0Backend backend) noexcept {
  ...
  if(q8_0_resolve_backend(backend)==Q8_0Backend::avx512_vnni) return "q4-g16-vnni512-16rows-8vectors";
#endif
...
void q4_g16_matmul_avx2(const Q4G16Tile*,const Q4G16ActivationTile*,float*,std::size_t,std::size_t,std::size_t,std::size_t) noexcept;
#endif
#if QWEN35X_Q8_0_HAS_AVX512_VNNI_TU
void q4_g16_matmul_evex(const Q4G16Tile*,const Q4G16ActivationTile*,float*,std::size_t,std::size_t,std::size_t,std::size_t) noexcept;
void q4_g16_matmul_wide(const Q4G16Tile*,const Q4G16ActivationTile*,float*,std::size_t,std::size_t,std::size_t,std::size_t) noexcept;
#endif
namespace {
...
void q4_g16_pack_activations(const Q8_0BlockX1* input,Q4G16ActivationTile* out,
    std::size_t count,std::size_t blocks) noexcept {
  for(std::size_t t=0;t<count;++t) for(std::size_t b=0;b<blocks;++b) {
    auto& dst=out[(t/16)*blocks+b];const auto& src=input[t*blocks+b];
    dst.scales[t%16]=src.scales[0];std::memcpy(dst.qs[t%16],src.qs,32);
    for(int half=0;half<2;++half) {
      int sum=0;for(int i=0;i<16;++i) sum+=src.qs[half*16+i];
      dst.sums[half][t%16]=static_cast<std::int16_t>(sum);
    }
  }
}
void q4_g16_matmul(const Q4G16Tile* w,const Q4G16ActivationTile* a,
  ...
  if(q8_0_resolve_backend(backend)==Q8_0Backend::avx512_vnni) {
    q4_g16_matmul_wide(w,a,out,rows,count,blocks,stride);return;
  }
#endif
#if QWEN35X_Q8_0_HAS_AVX2_TU
  if(q8_0_backend_uses_avx2(backend)) {
    q4_g16_matmul_avx2(w,a,out,rows,count,blocks,stride);return;
  }
#endif
  for(std::size_t t=0;t<count;++t) for(std::size_t r=0;r<rows;++r) {
    float sum=0;
    for(std::size_t b=0;b<blocks;++b) for(int half=0;half<2;++half) {
      const auto& v=a[(t/16)*blocks+b];int dot=0;
      for(int i=half*16;i<half*16+16;++i)
        dot+=packed_code(w[(r/8)*blocks+b],static_cast<int>(r%8),i)*v.qs[t%16][i];
      sum+=dot*(detail::half_to_float(w[(r/8)*blocks+b].d[half][r%8])*v.scales[t%16]);
    }
    out[t*stride+r]=sum;
  }
}
